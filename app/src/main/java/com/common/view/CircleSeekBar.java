...
public class CircleSeekBar extends SeekBar {
	...
	protected void onDraw(Canvas canvas) {
		...
		float right = canvas.getWidth() - paddingRight - mUnreachedWidth
				/ 2;
		float bottom = canvas.getHeight() - paddingBottom
				- mUnreachedWidth / 2;
		float centerX = (left + right) / 2;
		float centerY = (top + bottom) / 2;

		float wheelRadius = (canvas.getWidth() - paddingLeft - paddingRight)
				/ 2 - mUnreachedWidth / 2;
		...
		if (isHasCache) {
			...
			canvas.drawArc(new RectF(left, top, right, bottom), -180, 180,
					false, mWheelPaint);
		}

//		Log.d("aaa", "mCurProcess" + mCurProcess+":"+mCurAngle);
		// 画选中区域
		canvas.drawArc(new RectF(left, top, right, bottom), -180,
				(float) (360 - mCurAngle) + 90, false, mReachedPaint);
