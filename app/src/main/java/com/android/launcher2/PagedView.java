...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
	...
	protected int getRelativeChildOffset(int index) {
		if (mChildRelativeOffsets != null) {
			// Log.d("allen1",
			// mChildRelativeOffsets.length+"getRelativeChildOffset"+index+":"+getChildCount());
			if (index >= mChildRelativeOffsets.length) {
				Log.d("allen", mChildRelativeOffsets.length
						+ ">>>>>>>>>getRelativeChildOffset" + index);
				return 0;
			}
		}
    ...
    private void cancelPageMoving(){
		if (DragLayer.mMultiPointIgnore && mTouchState == TOUCH_STATE_SCROLLING) {
			mTouchState = TOUCH_STATE_REST;
			pageEndMoving();
    ...
    private void showPageIndex(int whichPage){
    	if (MachineConfig.VALUE_SYSTEM_UI20_RM10_1.equals(Utilities.mSystemUI) ||
    		...
    		if (R.id.workspace != getId())
    			return;
    	}

		for (int i = 0; i < 5; ++i) {
			View v = this.getRootView().findViewById(getPageId(i));
			if(v!=null){
				if(mNextPage==i){
		...
		//ww+
		if (MachineConfig.VALUE_SYSTEM_UI21_RM10_2.equals(Utilities.mSystemUI)) {
			View v = this.getRootView().findViewById(R.id.left_toolbar);
			if (v != null) {
				if (whichPage == 2)
					v.setVisibility(View.INVISIBLE);
				else
			...
			if (R.id.workspace != getId())
    			return;
			View v = this.getRootView().findViewById(R.id.hotseat);
			if (v != null) {
				if (whichPage != 0)
