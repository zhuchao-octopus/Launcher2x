...
public class IconCache {
    ...
    public void getTitleAndIcon(ApplicationInfo application,
                                LauncherActivityInfo info, HashMap<Object, CharSequence> labelCache) {
        synchronized (mCache) {
            CacheEntry entry = cacheLocked(application.componentName, info,
                    labelCache, info.getUser());
            WinceCEStyleApp.updateLabel(application.componentName, entry.title);
            Log.d("getTitleAndIcon", application.componentName + ":entry.title:" + entry.title);
    ...
    public Bitmap getIcon(Intent intent, UserHandle user) {
        synchronized (mCache) {
            LauncherApps launcherApps = (LauncherApps) mContext
                    .getSystemService(Context.LAUNCHER_APPS_SERVICE);
            final LauncherActivityInfo launcherActInfo = launcherApps
    ...
    private boolean ifNeedtoRound(String pkgName) {
        if (pkgName != null) {
            for (int i = 0; i < needRoundPkgName.length; i++) {
                if (pkgName.equals(needRoundPkgName[i]))
                    return true;
            }
        }
        return false;
        ...
        if (entry == null) {
            ...
            if (MachineConfig.VALUE_SYSTEM_UI_KLD5.equals(Utilities.mSystemUI)) {
                ...
                if (Utilities.mSystemUI != null && !Utilities.mSystemUI.equals(MachineConfig.VALUE_SYSTEM_UI_KLD11_200)) {
                    d = getIconBackground(key.getPackageName(),
                            key.getClassName());
...
//					}
                ...
                if (MachineConfig.VALUE_SYSTEM_UI20_RM10_1.equals(Utilities.mSystemUI) ||
                    ...
                    if (pkgName != null && ifNeedtoRound(pkgName)) {
...
//			}
            ...
            if ((MachineConfig.VALUE_SYSTEM_UI40_KLD90.equals(Utilities.mSystemUI) ||
                ...
                if (d == null) {
                    ...
                    int j = mContext.getResources().getIdentifier(s,
