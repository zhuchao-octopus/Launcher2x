...
public class DragController {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (DragLayer.mMultiPointIgnore && mDragging)	//ww+
        	cancelDrag();
        switch (action) {
    ...
    private void checkScrollState(int x, int y) {
      ...
      if(mLauncher.getWorkspace().getCurrentPage()==0){//0 page is full default by allen
        ...
        if (x < mScrollZone) {
            if (mScrollState == SCROLL_OUTSIDE_ZONE && mLauncher.getWorkspace().getCurrentPage()>1) {
                mScrollState = SCROLL_WAITING_IN_ZONE;
